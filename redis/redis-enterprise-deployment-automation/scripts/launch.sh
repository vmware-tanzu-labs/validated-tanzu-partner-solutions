...
CLUSTER_IMAGE=""
CLUSTER_NAME="${CLUSTER_NAME:=deployment-automation-cluster}"   # The name of the cluster to create with Kind
...
function deploy_tekton() {
  ...
  if ! kubectl apply --filename "${TEKTON_PIPELINE_FILE}"; then
    return 1
  fi
  if ! kubectl apply --filename "${TEKTON_TRIGGERS_FILE}"; then
    return 1
  fi
  printf "Done\n\n"
}
